  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black to-[#111111] text-white">
      ...
      <main className="pt-32 px-6 sm:px-12 max-w-4xl mx-auto">
        <h1 className="text-3xl sm:text-4xl font-bold mb-10 text-center text-white">Your Prompt History</h1>

        {history.length === 0 ? (
          <p className="text-center text-white/60 text-lg mt-12">There is no history.</p>
        ) : (
          <ul className="space-y-6">
            {history.map((entry, index) => (
              <li key={index} className="bg-white/5 p-6 rounded-2xl shadow-md hover:scale-[1.01] transition-all duration-300">
